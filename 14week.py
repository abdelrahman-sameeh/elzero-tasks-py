...
class User:
    def __init__(self, f_name, l_name, age, gender) -> None:
      self.f_name = f_name
      self.l_name = l_name
      self.age = age
      self.gender = gender
    ...
    def full_details(self):
      return f'Hello {"Mr" if self.gender == "Male" else "Mis"} {self.f_name}  {self.l_name[0]}. [{40-int(self.age)}] Years To Reach 40'
...
class Games:
  ...
  def show_games(self):
    if type(self.param) is str:
      print(f'I Have One Game Called "{self.param}"')
    elif type(self.param) is list:
      print('I Have Many Games:')
      for el in self.param:
        print(f'-- {el}')
    elif type(self.param) is int:
      print(f'I Have {self.param} Game.')
